...
import { formatNumber, formatBytes } from '../utils/formatUtils';
...
export class StatisticsCollector {
  aggregate(
    ...
    const potentialSpaceSaved = duplicateGroups.reduce((sum: number, group: DuplicateGroup) => {
      const duplicates = group.slice(1);
      return sum + duplicates.reduce((groupSum: number, file) => groupSum + file.size, 0);
    }, 0);
    const filesDeleted = deleteResults.filter((r: DeleteResult) => r.success).length;
    const deletionsFailed = deleteResults.filter((r: DeleteResult) => !r.success).length;
    return {
  ...
  displayScanResults(stats: Statistics, duplicateGroups: DuplicateGroup[]): void {
    console.log('\n✅ Scan completed\n');
    ...
    console.log(`Potential space saved: ${formatBytes(stats.potentialSpaceSaved)}\n`);
  ...
  displayGroups(groups: DuplicateGroup[]): void {
    ...
    for (const group of groups) {
      ...
      console.log('═══════════════════════════════════════');
      console.log(`[KEEP]   ${group[0].path} (${formatNumber(group[0].size)} bytes)`);
      for (let i = 1; i < group.length; i++) {
        console.log(`[DELETE] ${group[i].path} (${formatNumber(group[i].size)} bytes)`);
      }
      console.log('');
      groupNumber++;
    }
