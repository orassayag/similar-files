...
describe('StatisticsCollector', () => {
  ...
  describe('aggregate', () => {
    ...
    it('should calculate statistics for multiple duplicate groups', () => {
      const groups = [
        createGroup([1000, 1050, 1030]),
        createGroup([2000, 2050]),
      ];
      const stats = collector.aggregate(100, groups, []);
  ...
  describe('display methods', () => {
    it('should display scan results', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Files scanned: 100')
      );
    ...
    it('should display deletion results', () => {
      ...
      const stats = collector.aggregate(
        100,
        [],
        [{ success: true, path: 'test' }]
      );

      collector.displayDeletionResults(stats);

      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Deletion Summary')
      );
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Files deleted: 1')
      );
      consoleSpy.mockRestore();
    });
    ...
    it('should display failed deletions if any', () => {
      ...
      const stats = collector.aggregate(
        100,
        [],
        [{ success: false, path: 'test', error: 'err' }]
      );

      collector.displayDeletionResults(stats);

      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Deletions failed: 1')
      );
      consoleSpy.mockRestore();
    });
  });
