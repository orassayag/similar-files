...
describe('pathUtils', () => {
  describe('shouldIgnorePath', () => {
    ...
    it('should return true when path contains ignore pattern', () => {
      expect(
        shouldIgnorePath('/project/node_modules/package', ['node_modules'])
      ).toBe(true);
    });

    it('should return false when path does not contain ignore pattern', () => {
      expect(shouldIgnorePath('/project/src/main.ts', ['node_modules'])).toBe(
        false
      );
    });
    ...
    it('should match patterns anywhere in path', () => {
      expect(shouldIgnorePath('/a/b/node_modules/c/d', ['node_modules'])).toBe(
        true
      );
      expect(
        shouldIgnorePath('/node_modules/project/src', ['node_modules'])
      ).toBe(true);
    });
  });
});
