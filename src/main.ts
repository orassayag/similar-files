...
import { DuplicateFinder } from './core/duplicateFinder';
import { Deleter } from './core/deleter';
...
async function main() {
  ...
  const duplicateGroups = finder.findDuplicates(files, settings.sizeToleranceBytes);
  const statisticsCollector = new StatisticsCollector();
  const initialStats = statisticsCollector.aggregate(files.length, duplicateGroups, []);
  ...
  if (!settings.dryMode && duplicateGroups.length > 0) {
    ...
    const finalStats = statisticsCollector.aggregate(files.length, duplicateGroups, deleteResults);
